...
```
show access on microflow Administration.ChangeMyPassword;
```
...
```
show access on page Administration.Account_Overview;
```
...
> **Note:** The role name must be quoted. Unquoted names cause a parse error.
...
> Partial revoke (removing specific member access) works correctly as of v0.7.0.
...
> **Note:** Expected behavior depends on the project's password policy. If MinLength=1, weak passwords will be accepted without error.
