...
| App | Studio Pro | Microflows | Java Actions | JS Actions |
|-----|-----------|------------|--------------|------------|
...
for mpr in "$APPS_DIR"/*/*.mpr; do
  ...
  echo "show microflows;" > /tmp/show-mf.mdl
  mxcli exec /tmp/show-mf.mdl -p "$mpr" 2>&1 | tail -1
done
```

Expected: count line `(N microflows)` for each project.
...
> Dropped microflows cannot be recovered.
>
> ```bash
...
**Expected:** Empty result or `(0 microflows)`.
...
**Expected:** Excluded microflows show `Yes` in `Excluded` column.
...
```
describe microflow Administration.ACT_CreateAccount;
```

**Expected:** Roundtrippable MDL:
```
create or modify microflow Administration.ACT_CreateAccount (
  $AccountName: String
)
returns Void
begin
...
```
create microflow MyModule.MF_CountItems (
  $Items: List of MyModule.Item
)
returns Integer
begin
  declare $Count Integer = count($Items);
  return $Count;
end;
```

**Expected:** List parameter and count aggregate.
...
returns Void
begin
  call microflow MyModule.MF_ProcessOrder($Order);
end;
```

**Expected:** Call action referencing another microflow.

### 3.12 Call Java action
...
returns String
begin
  $Result = call java action MyModule.JA_Transform($Input);
  return $Result;
end;
```
...
### 3.13 Call JavaScript action

```
create microflow MyModule.MF_WithJS ()
returns Void
begin
  call javascript action MyModule.JSA_ShowNotification('Done');
end;
```

**Expected:** JavaScript action call.

### 3.14 REST call

```
create microflow MyModule.MF_ApiCall ()
returns String
begin
...
**Expected:** REST call activity (microflow-only).
...
returns Void
begin
  show page MyModule.Product_Detail($Product);
end;
```
...
begin
  on error continue
  begin
    call microflow MyModule.MF_Risky();
  end;
  return true;
end;
```

**Expected:** Error handling block wrapping activities.

### 3.19 Import/Export mapping

```
create microflow MyModule.MF_ImportData (
  $Json: String
)
returns MyModule.Data
begin
  $Result = import from mapping MyModule.DataMapping source $Json;
  return $Result;
end;
```

**Expected:** Import mapping activity (microflow-only).

### 3.20 Workflow actions
...
returns Void
begin
  call workflow MyModule.ApprovalWorkflow($Context);
end;
```

**Expected:** Workflow call activity (microflow-only).

### 3.21 `create or modify` — existing microflow
...
**Expected:** Updates existing microflow body.

### 3.22 `create or replace` — drop and recreate
...
### 3.23 Duplicate (without modifier)
...
**Expected:** Parameter shows `entity <TypeParamName>` for entity type selector.
...
returns String
begin
  $Result = call microflow MyModule.MF_Greet('World', 1);
  return $Result;
end;
```

**Expected:** Arguments passed positionally.
...
begin
  if $N <= 1 then
    ...
    $Prev = call microflow MyModule.MF_Recursive($N - 1);
    return $N * $Prev;
  end if;
end;
```
...
```
grant execute on microflow MyModule.MF_ProcessOrder to Manager, Admin;
```
...
```
revoke execute on microflow MyModule.MF_ProcessOrder from Manager;
```
...
grant execute on microflow MyModule.MF_CreateOrder to OrderClerk;
```
...
**Expected:** Error — referenced microflow not found.
...
**Expected:** Error or warning about type mismatch.
...
```
describe microflow MyModule.MF_Complex --format mermaid;
```
...
|---------------|-----------|---------|
| declare/set variable | 3.5 | CREATE |
| if/else | 3.6 | CREATE |
| loop | 3.7 | CREATE |
| create object | 3.8 | CREATE |
| commit (with events) | 3.3 | CREATE |
| retrieve (association) | 3.9 | CREATE |
| retrieve (XPath) | 3.10 | CREATE |
| call microflow | 3.11, 10.x | CREATE, CALL |
| call java action | 3.12 | CREATE |
| call javascript action | 3.13 | CREATE |
| rest call | 3.14 | CREATE |
| show page | 3.15 | CREATE |
| show message | 3.16 | CREATE |
| log message | 3.17 | CREATE |
| error handling | 3.18 | CREATE |
| import mapping | 3.19 | CREATE |
| workflow call | 3.20 | CREATE |
| return | 3.2, 3.5–3.10 | CREATE |
...
| 3.4 | CREATE | List param | | | | |
| 3.5 | CREATE | Variables | | | | |
| 3.6 | CREATE | If/else | | | | |
| 3.7 | CREATE | Loop | | | | |
| 3.8 | CREATE | Create object | | | | |
| 3.9 | CREATE | Retrieve assoc | | | | |
| 3.10 | CREATE | Retrieve XPath | | | | |
| 3.11 | CREATE | Call microflow | | | | |
| 3.12 | CREATE | Call Java | | | | |
| 3.13 | CREATE | Call JavaScript | | | | |
| 3.14 | CREATE | REST call | | | | |
| 3.15 | CREATE | Show page | | | | |
| 3.16 | CREATE | Show message | | | | |
| 3.17 | CREATE | Log message | | | | |
| 3.18 | CREATE | Error handling | | | | |
| 3.19 | CREATE | Import mapping | | | | |
| 3.20 | CREATE | Workflow call | | | | |
| 3.21 | CREATE | create or modify | | | | |
| 3.22 | CREATE | create or replace | | | | |
| 3.23 | CREATE | Duplicate error | | | | |
| 4.1 | DROP | Existing | | | | |
