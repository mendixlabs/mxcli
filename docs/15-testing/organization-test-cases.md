...
> **Note:** `SHOW STRUCTURE ALL IN <Module>` is valid — it shows the full-depth structure of a single module including marketplace/system content.
...
**Expected:** Returns `(no modules found)` — no error raised, just an empty result.
...
**Expected:** Parser error — not a friendly runtime error. Example: `MOVE LAYOUT MyModule.MyLayout TO Target;` produces a parse error because `LAYOUT` is not a recognized MOVE target.

> **Note:** The parser rejects unsupported MOVE types before execution, so the error message is a generic parse error rather than a descriptive "unsupported document type" message.
...
**Expected:** Succeeds silently with a misleading success message (e.g., "moved page") even though source and target module are the same. No actual change occurs.

> **Note:** The command does not detect same-module moves. It reports success without warning that the document was already in the target module.
...
### 12.4 SHOW STRUCTURE IN filters marketplace/system modules

```
show structure in Atlas_Core;
```

**Expected:** `SHOW STRUCTURE IN <Module>` filters out marketplace and system modules by default. To inspect these modules, use `SHOW STRUCTURE ALL` and locate the module in the full tree.
