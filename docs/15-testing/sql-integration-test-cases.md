...
| App | Studio Pro | Entities |
|-----|-----------|----------|
| Lato Enquiry Management | 11.4.0 | — |
| Evora - Factory Management | 10.24.15 | — |
| Lato Product Inventory | 11.2.0 | — |

> **SQL tests require external databases.** Provision PostgreSQL, MSSQL, and Oracle instances with test data before running. See §Setup below.
...
### 2. Provision test databases

Provision three databases with identical test schemas:

| Driver | Container / Service | DSN example |
|--------|-------------------|-------------|
| postgres | `docker run -d -p 5432:5432 -e POSTGRES_PASSWORD=test postgres:16` | `postgres://postgres:test@localhost:5432/testdb` |
| sqlserver | `docker run -d -p 1433:1433 -e ACCEPT_EULA=Y -e SA_PASSWORD=Test1234! mcr.microsoft.com/mssql/server:2022-latest` | `sqlserver://sa:Test1234!@localhost:1433?database=testdb` |
| oracle | `docker run -d -p 1521:1521 -e ORACLE_PASSWORD=test gvenzl/oracle-free:slim` | `oracle://system:test@localhost:1521/FREEPDB1` |

Seed each database:

```sql
CREATE TABLE users (id SERIAL PRIMARY KEY, name VARCHAR(100), email VARCHAR(200), active BOOLEAN, created_at TIMESTAMP);
CREATE TABLE orders (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id), total DECIMAL(10,2), status VARCHAR(50));
CREATE TABLE products (id SERIAL PRIMARY KEY, name VARCHAR(200), price DECIMAL(10,2), category VARCHAR(100));
CREATE VIEW active_users AS SELECT * FROM users WHERE active = true;
INSERT INTO users (name, email, active, created_at) SELECT 'User ' || g, 'user' || g || '@test.com', true, NOW() FROM generate_series(1, 1000) g;
INSERT INTO orders (user_id, total, status) SELECT (g % 1000) + 1, (random() * 1000)::decimal(10,2), CASE WHEN g % 3 = 0 THEN 'shipped' WHEN g % 3 = 1 THEN 'pending' ELSE 'cancelled' END FROM generate_series(1, 10000) g;
INSERT INTO products (name, price, category) SELECT 'Product ' || g, (random() * 100)::decimal(10,2), CASE WHEN g % 4 = 0 THEN 'A' WHEN g % 4 = 1 THEN 'B' WHEN g % 4 = 2 THEN 'C' ELSE 'D' END FROM generate_series(1, 200) g;
```

> Adapt syntax for MSSQL and Oracle where `generate_series` is unavailable.

### 3. Prepare connections.yaml

Create `.mxcli/connections.yaml` in the test project directory:
...
connections:
  - alias: mydb
    driver: postgres
    dsn: postgres://postgres:test@localhost:5432/testdb
  - alias: mssqldb
    driver: sqlserver
    dsn: sqlserver://sa:Test1234!@localhost:1433?database=testdb
  - alias: oradb
    driver: oracle
    dsn: oracle://system:test@localhost:1521/FREEPDB1
```

Also test map format:
...
### 6. Script-based testing

```bash
mxcli exec test-sql.mdl -p <mpr>
```

Write operations (CREATE DATABASE CONNECTION, GENERATE CONNECTOR EXEC, IMPORT FROM) modify the `.mpr` file **in place**.
...
> Changes from GENERATE CONNECTOR EXEC and IMPORT FROM cannot be easily undone.
>
> ```bash
...
### 1.8 Short form — from connections.yaml

```
SQL CONNECT mydb
```

**Expected:** `Connected to postgres database as 'mydb'` (resolved from `.mxcli/connections.yaml`).

### 1.9 Auto-connect resolution — env var MXCLI_SQL_<ALIAS>_DSN

```bash
export MXCLI_SQL_ENVDB_DSN='postgres://postgres:test@localhost:5432/testdb'
```
...
**Expected:** `Connected to postgres database as 'envdb'`

### 1.10 Auto-connect resolution — env var MXCLI_SQL_<DRIVER>_DSN

```bash
export MXCLI_SQL_POSTGRES_DSN='postgres://postgres:test@localhost:5432/testdb'
```

```
SQL CONNECT postgres_fallback
```

**Expected:** Resolves driver from env var. Connection succeeds.

### 1.11 Auto-connect resolution priority

Set all four sources (explicit DSN, alias env var, driver env var, connections.yaml) for the same alias. Connect with short form. Verify explicit DSN wins.

### 1.12 Unsupported driver
...
### 1.16 connections.yaml — list format

Use list format from §Setup. `SQL CONNECT mydb` resolves.

### 1.17 connections.yaml — map format

Use map format from §Setup. `SQL CONNECT mydb` resolves.

### 1.18 Auto-connect not found

```
SQL CONNECT nonexistent_alias
```

**Expected:** Error — no connection config found for alias.
...
```
IMPORT FROM mydb QUERY 'SELECT o.id, o.total, o.user_id FROM orders o LIMIT 100' INTO MyModule.Order MAP (id AS Id, total AS Total) LINK (user_id AS Order_User)
```
...
```
IMPORT FROM mydb QUERY 'SELECT o.id, o.total, o.user_id FROM orders o LIMIT 100' INTO MyModule.Order MAP (id AS Id, total AS Total) LINK (user_id AS Order_User ON UserId)
```
...
```
IMPORT FROM mydb QUERY 'SELECT ...' INTO MyModule.Entity MAP (...) LINK (col AS MyReferenceSet)
```
...
### 9.8 CREATE OR MODIFY — update existing

```
CREATE DATABASE CONNECTION MyModule.MyDB TYPE PostgreSQL CONNECTION STRING @MyModule.DSN BEGIN END;
CREATE OR MODIFY DATABASE CONNECTION MyModule.MyDB TYPE PostgreSQL CONNECTION STRING @MyModule.DSN BEGIN
  QUERY NewQuery SQL 'SELECT 1' RETURNS MyModule.Entity MAP (id AS Id);
END;
```

**Expected:** Connection updated. DESCRIBE shows `NewQuery`.

### 9.9 Database types — MSSQL
...
4. `IMPORT FROM mydb QUERY 'SELECT id, user_id, total FROM orders LIMIT 100' INTO TestModule.Order MAP (id AS Id, total AS Total) LINK (user_id AS Order_User ON UserId)`
...
```
IMPORT FROM mydb QUERY 'SELECT o.id, o.total, o.status, o.user_id, u.name AS user_name FROM orders o JOIN users u ON o.user_id = u.id' INTO MyModule.Order MAP (id AS Id, total AS Total, status AS Status) LINK (user_id AS Order_User ON UserId) BATCH 200 LIMIT 5000
```
...
| SQL CONNECT — auto-connect resolution | — | **Manual only** |
...
| CREATE OR MODIFY DATABASE CONNECTION | — | **Manual only** |
