...
| App | Studio Pro | CLI features exercised |
|-----|-----------|------------------------|
| Lato Enquiry Management | 11.4.0 | docker, test, check, bson, fmt |
| Evora - Factory Management | 10.24.15 | docker, playwright, report, widget |
| Lato Product Inventory | 11.2.0 | test, check, bson, lsp |
...
Docker Desktop must be running. Commands `docker`, `test`, and `playwright` require it.

### 3. Set environment

```bash
export APPS_DIR=<path-to-extracted-apps>
export MPR="$APPS_DIR/EnquiriesManagement/EnquiriesManagement.mpr"
```

> **IMPORTANT:** Commands `docker`, `test`, `fmt`, and `new` modify project files. Always run against a **copy** of the project folder.
>
> ```bash
> cp -r MyProject MyProject-test
> export MPR="MyProject-test/MyProject.mpr"
> ```
...
**Expected:** Creates `docker-compose.yml` and `Dockerfile` in the project directory. Exit code `0`.
...
**Expected:** Reports runtime status (running/stopped). Exit code `0` if healthy, `1` if unhealthy.
...
**Expected:** Exit code `1`. Error message indicates missing `.mpr` file.
...
**Expected:** Prints current auth state (logged in/out, username, token expiry). Exit code `0`.
...
**Expected:** Status reports "Not logged in". Exit code `0`.
...
```bash
mxcli marketplace search "email"
```

**Expected:** Prints matching marketplace modules with name, version, compatibility. Exit code `0`.

### 8.2 Module info

```bash
mxcli marketplace info "Email Connector"
```

**Expected:** Prints module details: description, author, latest version, supported Studio Pro versions. Exit code `0`.

### 8.3 List versions

```bash
mxcli marketplace versions "Email Connector"
```

**Expected:** Prints version history with release dates. Exit code `0`.
...
**Expected:** Prints "No results found." Exit code `0`.
...
```bash
mxcli widget extract MyWidget -p "$MPR"
```

**Expected:** Extracts widget package to project's `widgets/` directory. Exit code `0`.
...
**Expected:** Prints BSON document structure to stdout. Exit code `0`.
...
### 10.3 Compare two MPRs

```bash
mxcli bson compare "$MPR" "$APPS_DIR/FactoryManagement/FactoryManagement.mpr"
```

**Expected:** Prints structural diff between two MPR files. Exit code `0` if identical, `1` if different.
...
**Expected:** Reformats the MDL file in place. Exit code `0`.
...
**Expected:** Error: unsupported file type or parse error. Exit code `1`.

---

## 12. mxcli new

### 12.1 Scaffold new document

```bash
mxcli new microflow MyModule.ACT_DoSomething -p "$MPR"
```

**Expected:** Creates a new microflow stub in the project. Exit code `0`.

### 12.2 Scaffold with invalid type

```bash
mxcli new unknowntype Foo -p "$MPR"
```

**Expected:** Error: unknown document type. Exit code `1`.
...
**Expected:** Server starts. Health endpoint returns `200 OK`. Exit code `0` on clean shutdown.
...
### 16.1 Validate project

```bash
mxcli check -p "$MPR"
```

**Expected:** Runs MxBuild validation. Prints errors and warnings. Exit code `0` if no errors, `1` if errors found.

### 16.2 SARIF output

```bash
mxcli check -p "$MPR" --sarif results.sarif
```

**Expected:** Writes SARIF JSON to `results.sarif`. Exit code reflects validation result.

### 16.3 Check nonexistent project

```bash
mxcli check -p /tmp/nonexistent.mpr
```
...
```bash
mxcli report -p "$MPR" --format html -o report.html
```
...
| Invalid MPR path | `mxcli check -p /bad/path.mpr` | "file not found" | `1` |
| Docker not running | `mxcli docker run -p "$MPR"` | "cannot connect to Docker daemon" | `1` |
| MxBuild not installed | `mxcli check -p "$MPR"` | "MxBuild not found — run mxcli setup mxbuild" | `1` |
...
| `marketplace` | search, info, versions | 4 | 0 | — |
