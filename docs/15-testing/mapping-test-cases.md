...
> Data Transformers require Mendix 11.9+ (feature gate: `integration/data_transformer`). Only the Lato apps (11.x) may contain them. Evora (10.24.15) must not.
...
for mpr in "$APPS_DIR"/*/*.mpr; do
  ...
  echo "show import mappings;" > /tmp/show-im.mdl
  mxcli exec /tmp/show-im.mdl -p "$mpr" 2>&1 | tail -1
  echo "show export mappings;" > /tmp/show-em.mdl
  mxcli exec /tmp/show-em.mdl -p "$mpr" 2>&1 | tail -1
done
```
...
### 5.1 List all data transformers
```
show data transformers;
```
**Expected:** Table with columns: Qualified Name, Module, Name, Source, Steps. Steps column shows arrow-joined technology names (e.g. `JavaScript → OQL → JavaScript`).

### 5.2 Filter by module
```
...
### 5.4 Non-existent module
```
show data transformers in NonExistentModule;
```
**Expected:** Error message.

### 5.5 Steps column formatting
Verify steps display as arrow-joined technology names (tech names from steps, not expressions).

### 5.6 Feature gate — Mendix 10.x project
```
...
### 6.1 Simple data transformer
```
describe data transformer Module.SimpleTransformer;
```
**Expected:** Valid MDL output in the form:
```
create data transformer Module.SimpleTransformer source <SourceType> '<sourceJSON>' {
  <Technology> '<expression>';
};
```

### 6.2 Multi-line expression — dollar quoting
Verify multi-line expressions use dollar-quoting:
```
create data transformer Module.MultiLine source JSON '{}' {
  JavaScript $$
    const x = input;
    return x;
  $$;
...
### 6.3 Single-line expression — regular quoting
Verify single-line expressions use regular quotes:
```
create data transformer Module.SingleLine source JSON '{}' {
  OQL 'SELECT * FROM Module.Entity';
};
```

### 6.4 Multiple steps
```
describe data transformer Module.MultiStep;
```
**Expected:** Multiple technology/expression pairs in sequence:
```
create data transformer Module.MultiStep source JSON '{}' {
  JavaScript 'transform(input)';
  OQL 'SELECT * FROM Module.Entity';
  JavaScript 'format(input)';
};
```

### 6.5 Source type rendering
Verify source type appears correctly (e.g. `source JSON`, `source OQL`, etc.).

### 6.6 Non-existent data transformer
```
describe data transformer Module.DoesNotExist;
```
**Expected:** Clear error message.

### 6.7 Feature gate — Mendix 10.x project
```
mxcli repl -p <evora-10.24.15.mpr>
