...
| App | Studio Pro | Nav Profiles | Configurations | Languages |
|-----|-----------|--------------|----------------|-----------|
...
> Navigation and settings changes modify the `.mpr` in place.
>
> ```bash
...
**Expected:** Line shows `→ microflow Module.MicroflowName`.
...
**Expected:** Error — invalid language code.
...
**Expected:** Login page, menu, and not found page all absent from MDL output.
...
1. `describe navigation Responsive_web;` — save output
2. Modify: `create or replace navigation Responsive_web ...` with a different home page
3. `describe navigation Responsive_web;` — verify change
4. Execute saved MDL from step 1
5. `describe navigation Responsive_web;`

**Expected:** Output at step 5 matches step 1 — original home page restored.

### 17.2 Configuration roundtrip

1. `create configuration 'RoundtripTest' DatabaseType PostgreSQL;`
2. `describe settings configuration 'RoundtripTest';` — save output
3. `drop configuration 'RoundtripTest';`
4. Execute saved output from step 2
5. `describe settings configuration 'RoundtripTest';`
...
### 18.1 Many configurations

Create 20 configurations:

```
create configuration 'Config01' DatabaseType PostgreSQL;
create configuration 'Config02' DatabaseType PostgreSQL;
...
create configuration 'Config20' DatabaseType PostgreSQL;
```

**Expected:** All 20 created. `show settings configurations` lists all.

### 18.2 Many constants in one configuration
...
### 18.4 Navigation with many menu items
...
### 18.5 All navigation profiles at once

Verify `show navigation` lists all profile types (Responsive_web, Tablet_web, Phone_web, Native_phone, Native_tablet) when all exist.
...
| 17.2 | ROUNDTRIP | Configuration | | | | |
| 18.1 | BOUNDARY | Many configurations | | | | |
| 18.2 | BOUNDARY | Many constants | | | | |
| 18.3 | BOUNDARY | Long constant value | | | | |
| 18.4 | BOUNDARY | Many menu items | | | | |
| 18.5 | BOUNDARY | All nav profiles | | | | |
